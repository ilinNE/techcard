...
def another_user(django_user_model):
    return django_user_model.objects.create_user(username='AnotherUser', password='1234567')
...
def another_user_client(another_user_token):
    ...
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f'Bearer {another_user_token["access"]}')
    return client