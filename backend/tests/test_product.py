...
class TestProduct:
    ...
    def test_product_create__unauthorized(self, client):
        ...
        valid_data = {"name": "test_product", "price": 102}
    ...
    def test_product_create__valid_data(self, user_client):
        ...
        assert (
            Product.objects.count() == product_count + 2
        ), "Новый продукт не добавлен в базу данных"
    ...
    def test_product_create__invalid_data(self, user_client):
        ...
        assert (
            Product.objects.count() == product_count
    ...
    def test_product_get_list(
        self, user_client, product_1, product_2, foreign_product
    ):
        response = user_client.get(self.url)
        assert (
            response.status_code == 200
        ), "Неверный код ответа при запросе списка продуктов"
        assert len(response.json()) >= 2, "Список продуктов не полный"
        assert (
            len(response.json()) <= 2
